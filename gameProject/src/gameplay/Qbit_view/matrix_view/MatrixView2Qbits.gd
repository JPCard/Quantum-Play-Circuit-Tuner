...
func updateQbitSystem(matrix:Array)->void:
	...
	$QbitSystemLabel.text += "%2.1f + %2.1f" % [roundOutput(matrix[0][2].getReal()), roundOutput(matrix[0][2].getImaginary())] + " i" 
	$QbitSystemLabel.text += "\n\n" + "%2.1f + %2.1f" % [roundOutput(matrix[0][3].getReal()), roundOutput(matrix[0][3].getImaginary())] + " i"
